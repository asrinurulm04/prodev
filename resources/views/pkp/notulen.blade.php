              ...
              <table  id="" class="table table-striped table-bordered" style="width:100%">
                <thead>
                  <tr style="font-weight: bold;color:white;background-color: #2a3f54;">
                    ...
                    <th class="text-center" style="min-width:115px">Launch Deadline</th>
                    <th class="text-center" style="min-width:150px">Forecast</th>
                    <th class="text-center" style="min-width:230px"><input type="text" id="note2" readonly class="hilang text-center" style="min-width:200px"></th>
                    <th class="text-center">Priority</th>
                    <th class="text-center" style="min-width:115px">Launch Deadline</th>
                    <th class="text-center" style="min-width:150px">Forecast</th>
                    <th class="text-center" style="min-width:230px"><input type="text" id="note3" readonly class="hilang text-center" style="min-width:200px"></th>
                    <th class="text-center">Priority</th>
                    <th class="text-center" style="min-width:115px">Launch Deadline</th>
                    <th class="text-center" style="min-width:150px">Forecast</th>
                  </tr>
                ...
                <tbody class="sortable-list">
                  ...
                  <!-- perbandingan 1 -->
                  <tr >
                    <input type="hidden" name="note[{{$loop->index}}][pkp]" value="{{$Dpkp->id_project}}">
                    <?php $date = Date('j-F-Y'); ?>
                      ...
                      <table>
                        <tr>
                          ...
                          <td>
                            ...
                            @elseif($Dpkp->years==NULL)
                            <input style="max-width:90px" maxlength="4" minlength="4" type="number" name="note[{{$loop->index}}][years]" class="form-control" placeholder="Years" id="tahun">
                            @endif
                          </td>
                        </tr>
                      </table>
                      ...
                      @foreach($for as $forcast)
                        ...
                        <table class=""> 
                          <tr>
                            ...
                            <td><input style="max-width:125px" type="number" name="for[{{$loop->index}}][for]" class="form-control" value="{{$forcast->forecast}}"></td>
                    ...
                    <?php $B =  date('F'); ?>
                    <?php $T =  date('Y'); ?>
                    <input type="hidden" name="bulan" id="bulan" value="{{$B}}">
                    <input type="hidden" name="bulan" id="bulan" value="{{$B}}">
                    ...
                    <td><textarea cols="30" name="note[{{$loop->index}}][note]" class="col-md-12 col-sm-12 col-xs-12" rows="4">-
                      @foreach($notulen as $not)
                        @if($not->id_pkp==$Dpkp->id_project)
                          @if($not->note_pv_marketing!=NULL && $not->Bulan==$B && $info==2) {{$not->note_pv_marketing}}
                          @elseif($not->note_rd_pv!=NULL && $not->Bulan==$B && $info==1) {{$not->note_rd_pv}}
                          @endif
                        @endif
                      @endforeach
                    </textarea></td>
                    ...
                    <td>
                      @foreach($notulen as $not)
                        @if($not->id_pkp==$Dpkp->id_project)
                          @if($not->note_pv_marketing!=NULL && $not->Bulan==$B && $info==1)
                          ...
                          @elseif($not->note_rd_pv!=NULL && $not->Bulan==$B && $info==2)
                    ...
                    <td class="text-center">
                      @foreach($notulen as $not)
                          @if($not->id_pkp==$Dpkp->id_project)
                            @if($not->note_pv_marketing!=NULL && $not->Bulan==$B && $info==1) {{$not->prioritas}}
                            @elseif($not->note_rd_pv!=NULL && $not->Bulan==$B && $info==2) {{$not->prioritas}}
                            @endif
                          @endif
                      @endforeach
                    </td>
                    <td class="text-center">
                      @foreach($notulen as $not)
                        @if($not->id_pkp==$Dpkp->id_project)
                          @if($not->note_pv_marketing!=NULL && $not->Bulan==$B && $info==1) {{$not->launch}} {{$not->launch_years}} 
                          @elseif($not->note_rd_pv!=NULL && $not->Bulan==$B && $info==2) {{$not->launch}} {{$not->launch_years}} 
                          @endif
                        @endif
                      @endforeach
                    </td>
                    ...
                    @foreach($NForecast as $nf)
                      @if($nf->id_pkp==$Dpkp->id_project)
                        @if($info=='1' && $nf->info==2) {{$nf->satuan}} = {{$nf->forecash}}<br>
                        @elseif($info=='2' &&$nf->info==1) {{$nf->satuan}} = {{$nf->forecash}}<br>
                        @endif
                      @endif
                    @endforeach
                    </td>
                    <!-- Selesai -->
                    <!-- info note PV & Marketing -->
                    <td>
                      @foreach($notulen as $not)
                      @if($not->id_pkp==$Dpkp->id_project)
                        @if($not->note_pv_marketing!=NULL && $not->Bulan==$M)
                        <textarea cols="30"disabled value="{{$not->note_pv_marketing}}" class="col-md-12 col-sm-12 col-xs-12" rows="3"> {{$not->note_pv_marketing}}</textarea>
                        <input type="hidden" id="dua" value="PV & Marketing = {{$not->created}}">
                        @endif
                      @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($notulen as $not)
                        @if($not->id_pkp==$Dpkp->id_project)
                          @if($not->note_pv_marketing!=NULL && $not->Bulan==$M) {{$not->prioritas}} @endif
                        @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($notulen as $not)
                        @if($not->id_pkp==$Dpkp->id_project)
                          @if($not->note_pv_marketing!=NULL && $not->Bulan==$M) {{$not->launch}} {{$not->launch_years}} @endif
                        @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($NForecast as $nf)
                        @foreach($notulen as $not)
                          @if($nf->id_pkp==$not->id_pkp)
                            @if($nf->id_pkp==$Dpkp->id_project)
                              @if($not->note_pv_marketing!=NULL && $nf->Bulan==$M) 
                                @if($not->created==$nf->date) {{$nf->satuan}} = {{$nf->forecash}}<br>@endif
                              @endif
                            @endif
                          @endif
                        @endforeach
                    ...
                    <td>
                      @foreach($notulen as $not)
                      @if($not->id_pkp==$Dpkp->id_project)
                        @if($not->note_rd_pv!=NULL && $not->Bulan==$M) 
                        <textarea cols="30"disabled value="{{$Dpkp->not->note_rd_pv}}" class="col-md-12 col-sm-12 col-xs-12" rows="3"> {{$not->note_rd_pv}}</textarea>
                        <input type="hidden" id="tiga" value="PV & RD = {{$not->created}}">
                        @endif
                      @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($notulen as $not)
                      @if($not->id_pkp==$Dpkp->id_project)
                        @if($not->note_rd_pv!=NULL && $not->Bulan==$M) {{$not->prioritas}}
                        @endif
                      @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($notulen as $not)
                      @if($not->id_pkp==$Dpkp->id_project)
                        @if($not->note_rd_pv!=NULL && $not->Bulan==$M) {{$not->launch}} {{$not->launch_years}}
                        @endif
                      @endif
                      @endforeach
                    </td>
                    <td>
                      @foreach($NForecast as $nf)
                        @foreach($notulen as $not)
                          @if($nf->id_pkp==$not->id_pkp)
                            @if($nf->id_pkp==$Dpkp->id_project)
                              @if($not->note_rd_pv!=NULL && $nf->Bulan==$M) @if($not->created==$nf->date) {{$nf->satuan}} = {{$nf->forecash}}<br>@endif
                              @endif
                            @endif
                          @endif
                        @endforeach
...
<!-- /page content -->
  ...
  <script>
    ...
    document.getElementById('note2').value = dua;
    var tiga = document.getElementById('tiga').value;
    document.getElementById('note3').value = tiga;
  </script>
