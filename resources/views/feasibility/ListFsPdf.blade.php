...
<div class="x_panel">
  ...
  <div class="row">
    <div class="col-md-5">
      <table>
        <tr><th width="10%">Project Name</th><th width="45%">: {{$pdf->project_name}}</th>
        <tr><th width="10%">PKP Number</th><th width="45%">: {{$pdf->pdf_number}}{{$pdf->ket_no}}</th>
        <tr><th width="10%">Brand</th><th>: {{$pdf->id_brand}}</th>
      </table><br>
    </div>
    <div class="col-md-7">
      <table>
        <tr><th width="10%">Idea</th><th width="45%">: {{$pdf->background}}</th></tr>
        <tr><th width="10%">Configuration</th><th>: {{$pdf->id_brand}}</th>
        @if(auth()->user()->Departement->dept === 'REA')
        <tr><th width="10%">User Proses</th><th>: @if($pdf->user_fs!=NULL) {{$pdf->proses->name}} @endif</th>
        @endif
      </table><br>
    </div>
  </div>
          ...
          <tbody>
            @foreach($fs as $fs)
              ...
              @endif
                ...
                <td class="text-center">@foreach($lokasi2 as $lk2)  @if($fs->id_wb_kemas!=NULL) {{$lk2->IO}}, @endif <br>@endforeach</td>
                ...
                <td class="text-center">
                  @if($fs->status_feasibility=='pengajuan')
                    @if($pdf->user_fs!=NULL)
                    <a href="{{route('DetailPengajuanFsPDF',[$fs->id_project_pdf,$fs->id_formula,$fs->id])}}" class="btn btn-sm btn-dark" type="button"><li class="fa fa-file"></li> Pengajuan</a>
                    @endif
                ...
                <!-- Action user product -->
                <td class="text-center">
                  <a href="{{ route('formula.detail',[$fs->workbook->id,$fs->id_project_pdf,$fs->workbook->workbook_pdf_id]) }}" class="btn btn-sm btn-info" type="button" title="Show"><li class="fa fa-folder"></li></a>
                </td>
                <!-- Action user Proses -->
                <td class="text-center">
                  @if(auth()->user()->Departement->dept === 'REA')
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="form-group row">
                        ...
                        <label class="control-label text-bold col-md-3 col-sm-3 col-xs-12 text-center">Biaya Transport/UOM*</label>
                        <div class="col-md-9 col-sm-9 col-xs-12">
...
<!-- modal -->
<div class="modal" id="sent{{$pdf->id_project_pdf}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog " role="document">
    <div class="modal-content">
      <div class="modal-header">                 
        <h3 class="modal-title" id="exampleModalLabel">Sent Feasibility
      ...
      <div class="modal-body">
        <form class="form-horizontal form-label-left" method="POST" action="{{ Route('user_fs',$pdf->id_project_pdf)}}" novalidate>
        <div class=" row">
          <label class="control-label text-bold col-md-2 col-sm-2 col-xs-12 text-center"> User</label>
          <div class="col-md-9 col-sm-9 col-xs-12">
            <input type="hidden" name="info" id="info" value="PDF">
