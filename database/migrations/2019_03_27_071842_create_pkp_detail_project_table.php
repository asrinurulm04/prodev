...
class CreatePkpDetailProjectTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('pkp_detail_project', function (Blueprint $table) {
            ...
            $table->enum('status_by_rd_kemas',['ON PROGRESS', 'DONE'])->nullable();
            $table->enum('status_by_rd_produk',['ON PROGRESS', 'DONE'])->nullable();
            $table->enum('status_by_rd_process',['ON PROGRESS', 'DONE'])->nullable();
    ...
    {
        Schema::dropIfExists('pkp_detail_project');
    }
}
