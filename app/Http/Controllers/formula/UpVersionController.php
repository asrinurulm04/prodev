...
class UpVersionController extends Controller
{
    ...
    public function upversion(Request $request,$for,$wb,$pro){  //naik versi
        $lastf=Formula::where('id', $for)->first();
        if($lastf->workbook_id!=NULL){ // jika workbook_id tidak kosong maka bagian ini yang akan di jalankan
        ...
        if($lastf->workbook_pdf_id!=NULL){ //jika workbook_pdf_id tidak kosng maka bagian ini yang akan di jalankan
        ...
        $formulas = new Formula; // membuat formula baru
        ...
        $overage = new Overage; // membuat data overage baru
        $overage->id_formula=$formulas->id;
        $overage->save();

        $overage = new HeaderFormula; // menambah data header baru
        $overage->id_formula=$formulas->id;
		$overage->save();
        
        $clf=Fortail::where('formula_id',$lastf->id)->count(); // menghitung jumlah fortail dari formula yang dipilih
        if($clf>0){
            $lfortail=Fortail::where('formula_id',$lastf->id)->get();
            foreach ($lfortail as $lastft) { // menduplikasi data fortail dari formula
                ...
                $allergen= AllergenFormula::where('id_formula',$lastf->id)->count(); // menghitung jumlah allergen dari formula yang dipilih
                if($allergen>0){
        ...
        if(auth()->user()->role->namaRule == 'manager'){ // jika manager melakukan perubahan, maka user akan mendapatkan notifikasi via email.
            try{
    ...
    public function upversion2(Request $request,$for,$wb,$pro){ // naik subversi vormula
        $lastf = Formula::where('id',$for)->first();
        if($lastf->workbook_id!=NULL){
            $pkp  = PkpProject::where('id_pkp',$pro)->get();
            $data = PkpProject::where('id_pkp',$pro)->max('workbook')+1;
