...
class FormulaController extends Controller
{
    ...
    public function new(Request $request){     
        ...
        $formulas->formula 		   	 = $request->formula;
				...
				$formulas->berat_jenis	   = $request->berat_jenis;
					if($request->kategori_formula!=NULL){ // jika kategori tidak null maka masuka data sesuai request
						$formulas->kategori		 = $request->kategori_formula;
					}else{ // jika kategori null maka masukann "fg"
						$formulas->kategori		 = 'fg';
					}
        $formulas->versi 					 = 1;   
				$formulas->save();
		
        $overage = new HeaderFormula; // buat header untuk formula
        ...
        $header->id_formula = $formulas->id;
				...
				if($request->workbook_id!=NULL){ // jika workbook_id tidak null maka bagian ini akan dijalankan
				...
				}else{ // sedangkan juka workbook_id null maka bagian ini yang akan di jalankan
	...
	public function detail($for,$id,$pro){
        ...
        if($ada < 1){ // jika jumlah fortail kurang dari 1 maka akan menampilkan notifikasi error
      ...
      foreach($fortails as $fortail){// Get Persen
      ...
      $no = 0;
      foreach($fortails as $fortail){ //Get Needed
        ...
        //perhitungan nutfact bayangan
				...
				// Mikro
				$total_Yeast 		  	= $total_Yeast + $Yeast;							$total_Enterobacter = $total_Enterobacter + $Enterobacter;
      ...
      return view('formula/detailformula', compact('data','id'  ))->with([
				'idf' 			 		 => $idf,
				'ada'     	 		 => $ada,
				'formula'		 		 => $formula,
				'hfile' 		 		 => $hfile,
	...
	public function uploadfile(Request $request,$id){ // menambahkan data upload untuk workbook PKP
	...
	public function hapus_upload($id){ // menghapus data yang telah di upload
		$project = PkpProject::where('id_project',$id)->first();
		$project->file=NULL;
		$project->save();
		return redirect::back();
	...
	public function uploadfile_pdf(Request $request,$id){ // menambahkan data upload untuk workbook PDF
		$data = $request->file('filename');
		$nama = $data->getClientOriginalName();
	...
	public function hapus_upload_pdf($id){ // menghapus data yang telah di upload
		$project = ProjectPDF::where('id_project_pdf',$id)->first();
		$project->file=NULL;
		$project->save();
		return redirect::back();
