...
class SummaryFormulaController extends Controller
{
    ...
    public function summarry($for,$pkp,$pro){
        ...
        foreach($fortails as $fortail){
          ...
          $persen 		= $fortail->per_serving / $satu_persen; $persen = round($persen,2);
					if($fortail->nama_sederhana != 'NULL'){ //perhitungan nutfact bayangan
						// makro
        ...
        $total_harga = collect([	
          ...
          'total_harga_per_gram'	 => $total_harga_per_gram,	'total_berat_per_serving'=> $total_berat_per_serving,
          'total_persen'			 		 => 100,										'total_harga_per_serving'=> $total_harga_per_serving,
          'total_berat_per_batch'  => $total_berat_per_batch,	'total_harga_per_batch'  => $total_harga_per_batch,
          'total_berat_per_kg'	 	 => $total_berat_per_kg,		'total_harga_per_kg'	 	 => $total_harga_per_kg,                       
				]);
        
        return view('formula/summarry')->with([
          'ada'     		 		=> $ada,
          'formula' 		 		=> $formula,
		...
		public function overage(Request $request,$id){
			...
			$overage = Overage::where('id_formula',$id)->first(); // update data overage
    ...
    public function header(Request $request,$formula){ // membuat header bisa ditampilkan sesuai dengan yang di perlukan
