...
class Step2Controller extends Controller
{
    ...
    public function create($formula,$pkp,$project){
        ...
        if($formula->batch != null){ // checkbase !
            if($formula->batch != 0){
        ...
        foreach($fortails as $fortail){
            ...
            if($fortail->granulasi == 'ya'){// Granulasi
            $granulasi = $granulasi + 1;     
            }       
            if($fortail->premix == 'ya'){// premix
                $premix = $premix + 1;         
            } 
        }
    ...
    public function update($formula,$id,Request $request){ // update note
    ...
    public function insert($vf,Request $request){ //  menambahkan data formula/bahan
        ...
        $ada= Fortail::where('formula_id',$vf)->count();  // checkfortail !
        if($ada>0){            
        ...
        if (isset($request->cbase)){  // Jika Base Masih Kosong
            $batch   = $request->per_batch;
            $serving = $request->per_serving;
            $mybase  = $batch / $serving;

            if($ada>0){  // Check and fill all batch for fortail that doesnt have batch
        ...
        if (isset($request->cgranulasi)){ // Granulasi
            $fortails->granulasi = 'ya';                
        }else{
            $fortails->granulasi = 'tidak';
        }
        if (isset($request->cpremix)){ // premix
            $fortails->premix = 'ya';                
        }else{
        ...
        if(auth()->user()->role->namaRule == 'manager'){ // jika manager RD melakukan perubahan pada formula, maka user akan mendapatkan informasi via email
            try{
    ...
    public function hapusall($formula){ // menghapus data keseluruhan
        ...
        $allergen = AllergenFormula::where('id_formula',$formula)->delete(); // menghapus data allergen formula

        return redirect::back();
    }

    public function destroy($id,$vf){ // hapus per formula
        $formula    = Formula::where('id',$vf)->first();
