...
class Step1Controller extends Controller
{
    ...
    public function create($formula,$pkp,$project){ // halaman step 1 PKP
        $id_for  = $formula;
        $id_pkp  = $pkp;
        $id_pro  = $project;
    ...
    public function step1_pdf($formula,$pro){ // halaman step 1 PDF
        $id_for  = $formula;
        $id_pro  = $pro;
        $formula = Formula::where('id',$formula)->first();
        $data    = DataFormula::where('id_formula',$formula)->get();
    ...
    public function update(Request $request, $formula,$wb,$project){ //update data
        $id_for  = $formula;
        $formula = Formula::where('id',$formula)->first();
        ...
        if($request->file('filename')!=''){ // menambahkan upload file
            $files = [];
    ...
    public function hapus_file($id){ // hapus file
