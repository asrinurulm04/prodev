...
{
    public function template(Request $request,$id_ProjectPDF){ // membuat template untuk project PDF
        ...
        if($clf>0){ // menghitung jumlah sub PDF dengan id yang sama
            $isipdf=SubPDF::where('pdf_id',$id_ProjectPDF)->where('revisi',$max)->where('turunan',$pdf)->get();
            foreach ($isipdf as $ipdf){ // menduplikasi data project PDF
                $tip= new SubPDF;
    ...
    public function templatepkpumum($id_project,$revisi,$turunan,$kemas){ // tempate PKP umum
    ...
    public function templatepkpkemas($id_project,$revisi,$turunan,$kemas){ // membuat template project PKP kemas
        $pkp     = PkpProject::max('id_pkp')+1;
        $current = Carbon::now();
        $pkpp    = PkpProject::where('id_pkp',$id_project)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->first();
    ...
    public function templatepkpbaku($id_project,$revisi,$turunan,$kemas){ // template project PKP baku
        $pkp     = PkpProject::max('id_pkp')+1;
        $current = Carbon::now();
        $pkpp    = PkpProject::where('id_pkp',$id_project)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->first();
    ...
    public function improve(Request $request, $id_project,$revisi,$turunan,$kemas){ // menduplikais project eksis untuk di improve
        $pkp     = PkpProject::max('id_pkp')+1;
        $current = Carbon::now();
        ...
        $tip->kategori              = 'New';
        $tip->info                  = '_Imp';
        if($request->imp=='umum'){ // jika project nya umum maka seluruh data dari project yang dipilih akan di duplikasi
            $tip->jenis             = 'Umum';
            $tip->primery           = $pkpp->primery;
            $tip->secondary         = $pkpp->secondary;
            $tip->tertiary          = $pkpp->tertiary;
            $tip->kemas_eksis       = $pkpp->kemas_eksis;
            $tip->primer            = $pkpp->primer;
        }elseif($request->imp=='baku'){ // jika project yang dipilih adalah project baku, maka data kemasan dari project yang dipilih tidak akan ikut terduplikasi
            $tip->jenis             = 'Baku';
        }
        $tip->created_date          = $current->format('j-F-Y');
        $tip->last_update           = $current->format('j-F-Y');
    ...
    public function upversionpkp($id_project,$revisi,$turunan,$kemas){ // naik versi project PKP ketika melakukan revisi
        ...
        $datases=DataSES::where('id_project',$id_project)->where('id_pkp',$pkpp->id_pkp)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->count();
        if($datases>0){
            $isises=DataSES::where('id_project',$id_project)->where('id_pkp',$pkpp->id_pkp)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->get();
            foreach ($isises as $isises){
                $data1= new DataSES;
                $data1->id_project   = $tip->id_project;
        ...
        $datafor=Forecast::where('id_project',$id_project)->where('id_pkp',$pkpp->id_pkp)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->count();
        if($datafor>0){
            $isifor=Forecast::where('id_project',$id_project)->where('id_pkp',$pkpp->id_pkp)->where('revisi',$revisi)->where('turunan',$turunan)->where('revisi_kemas',$kemas)->get();
            foreach ($isifor as $isifor){
                $for= new Forecast;
    ...
    public function upversionpdf($id_ProjectPDF,$revisi,$turunan){ // naik versi project PDF ketika terjadi perubahan data setelah project di kirim ke RD
    ...
    public function upversionpromo($id_pkp_promo,$revisi,$turunan){ // naik versi project PKP promo ketika terjadi perubahan data setelah project di kirim ke RD
