...
class TabulasiController extends Controller
{
    ...
    public function edittabulasi(){
        $form = Tabulasi::where('user',Auth::user()->id)->limit('1')->first();
        $bahan = Bahan::join('tr_makro_bb','tr_makro_bb.id_bahan','ms_bahans.id')
        ->join('tr_vitamin_bb','tr_vitamin_bb.id_bahan','ms_bahans.id')
        ->join('tr_mineral_bb','tr_mineral_bb.id_bahan','ms_bahans.id')
    ...
    public function hapustabulasibb(){
        $check = EditProject::where('id_bahan','!=','NULL')->where('id_user',Auth::user()->id)->delete();
        $bahan = Tabulasi::where('user',Auth::user()->id)->delete();
    ...
    public function update_bahan(Request $request) {
        ...
        foreach($scores as $row){
            $makro = MakroBB::where('id_bahan',$row['id'])->update([
                "id_bahan" => $row['id'],
            ...
            $mineral = MineralBB::where('id_bahan',$row['id'])->update([
                "id_bahan" => $row['id'],
                "ca" => $row['ca'],
                "mg" => $row['mg'],
                "k" => $row['k'],
            ...
            $vitamin = VitaminBB::where('id_bahan',$row['id'])->update([
                "id_bahan" => $row['id'],
                "vitA" => $row['vitA'],
                "vitB1" => $row['vitB1'],
                "vitB2" => $row['vitB2'],
            ...
            $makro = AsamAminoBB::where('id_bahan',$row['id'])->update([
                "id_bahan" => $row['id'],
